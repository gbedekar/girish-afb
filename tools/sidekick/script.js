...
window.onload = async function () {
  ...
  if (type != null && type === "edit") {
    ...
    const sitePage = await getSitePageFromPath();
...
async function getSitePageFromPath() {
  const helixStatusResponse = await fetch(
    `https://admin.hlx.page/status/${owner}/${repo}/${ref}?editUrl=${referrer}`
  );
...
function displayTable(title) {
  ...
  const tableData = [
    ["Form"],
    [`https://${ref}--${owner}--${repo}.hlx.page/forms/${title}.json`],
  ];
