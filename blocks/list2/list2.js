    ...
    if (
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        if (
          ...
          const avgOkay = Math.round(
            (lcpOkay + clsOkay + fidOkay + inpOkay) / 4
          );
          const avgGood = Math.round(
            (lcpgood + clsgood + fidgood + inpgood) / 4
          );
          ...
          for (let j = 0; j < 7; j += 1) {
            ...
            if (cols[j] === "usrexp") {
              ...
              let txtContent;
              if (cols[j] === "avglcp") {
                txtContent = data[i][cols[j]] / 1000.0;
              } else if (cols[j] === "url") {
                listGridColumn.innerHTML = `<a href='${
                  data[i][cols[j]]
                }' target="_blank">${data[i][cols[j]].replace(
                  /^https?:\/\/[^/]+/i,
                  ""
                )}</a>`;
              } else if (cols[j] === "pageviews") {
                ...
                listGridColumn.innerHTML = `<a href="/views/rum-pageviews?${params.toString()}">${parseInt(
                  data[i][cols[j]],
                  10
              ...
              if (j >= 3) {
                if (
                  data[i][cols[j]] &&
                  data[i][cols[j]] <= ranges[cols[j]][0]
                ) {
                  listGridColumn.classList.toggle("pass");
                } else if (
                  data[i][cols[j]] > ranges[cols[j]][0] &&
                  data[i][cols[j]] < ranges[cols[j]][1]
                ) {
              ...
              if (txtContent) {
                if (j >= 3) {
                  const numb = parseFloat(txtContent)
                    .toFixed(2)
                    .toLocaleString("en-US");
                  const displayedNumb = numb.endsWith(".00")
                    ? numb.replace(".00", "")
                    : numb;
                  listGridColumn.textContent = `${displayedNumb}${
                    metrics[j - 3]
                  }`;
                } else {
        ...
        if (counter === 0) {
          ...
          if (params.has("domainkey") && params.has("url")) {
            ...
            noresults.innerHTML =
              '<i>domainkey</i> and <i>url</i> (hostname) are required.  Please provide <a href="/">here</a>.';
          }
